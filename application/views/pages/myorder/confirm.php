				...
				<div class="card-body">
					<?= form_open_multipart(base_url('myorder/confirm/' . $order['invoice'])) ?>
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" name="account_number" value="<?= set_value('account_number')?>">
							<?= form_error('account_number', '<small class="form-text text-danger">', '</small>') ?>
						</div>
						<div class="form-group">
							<label>Nominal</label>
							<input type="text" class="form-control" name="nominal" value="Rp. <?= $order['total'] ?>" readonly>
